import { communityArea, settlements } from "./settlements";
...
export function createAccessService({key, fetcher = fetch, now = Date.now, limit = 24}: Dependencies) {
  ...
  return async (input: unknown): Promise<Result> => {
    ...
    if (!input || typeof input !== "object") return fail(400, "Choose a pilot settlement and category.");
    const {settlementId, category} = input as Record<string, unknown>;
    const settlement = settlements.find(s => s.id === settlementId);
    if (!settlement || (category !== "medical" && category !== "pharmacy")) return fail(400, "Choose a pilot settlement and category.");
    ...
    try {
      const response = await fetcher("https://areainsights.googleapis.com/v1:computeInsights", {
        ...
        body: JSON.stringify({insights:["INSIGHT_COUNT"],filter:{
          locationFilter:{customArea:{polygon:{coordinates:communityArea(settlement).map(point=>({latitude:point.lat,longitude:point.lng}))}}},
