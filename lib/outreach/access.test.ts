...
const input = { regionId: "pasni", category: "pharmacy" };
...
describe("outreach count service", () => {
  ...
  it("never queries arbitrary locations or categories", async () => {
    ...
    for (const invalid of [null, {}, {...input, regionId:"unknown"}, {...input, category:"restaurant"}]) {
  ...
  it("queries the exact published tehsil boundary drawn on the map", async () => {
    const fetcher = upstream({count:"1"});
    expect((await createAccessService({key:"test", fetcher})(input)).status).toBe(200);
    ...
    const coordinates = boundaries.features[0].geometry.coordinates[0];
    expect(ring).toEqual(coordinates.map(([longitude, latitude]) => ({ latitude, longitude })));
    expect(ring.length).toBeGreaterThan(7);
    expect(ring[0]).toEqual(ring.at(-1));
    let signedArea = 0;
    for(let i=0;i<ring.length-1;i++) signedArea += ring[i].longitude*ring[i+1].latitude-ring[i+1].longitude*ring[i].latitude;
